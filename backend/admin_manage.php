...
<div class="space-y-8 max-w-5xl mx-auto">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        <div>
            ...
            <p class="text-slate-500 text-sm mt-1">Manage your admin profile, update credentials, and manage team accounts.</p>
        </div>
    ...
    <!-- CARD 1: MY ADMIN PROFILE & SECURITY SETTINGS (FOR LOGGED-IN ADMIN) -->
    <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-6">
        <div class="flex items-center justify-between border-b border-slate-100 pb-4">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-0.5">Edit your display name, email address, and reset your password while logged in.</p>
            </div>
            <span class="px-2.5 py-1 rounded-full text-xs font-bold bg-slate-100 text-slate-700 border border-slate-200">
                Logged in as: <strong class="text-slate-900"><?php echo htmlspecialchars($currentAdmin['username']); ?></strong>
            </span>
        </div>
        ...
        <form method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="my_name" placeholder="Full Display Name" value="<?php echo htmlspecialchars($currentAdmin['name'] ?? ''); ?>"
                ...
                <div>
                    ...
                    <input type="email" name="my_email" required placeholder="admin@ohmygudness.in" value="<?php echo htmlspecialchars($currentAdmin['email'] ?? ''); ?>"
                        class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-amber-500 outline-none">
                    <span class="text-[11px] text-slate-400 mt-1 block">Used for OTP password reset emails.</span>
                </div>
            </div>
            ...
            <div class="pt-4 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="password" name="current_password" required placeholder="••••••••"
                        class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-amber-500 outline-none">
                    <span class="text-[11px] text-slate-400 mt-1 block">Required to confirm any profile changes.</span>
                </div>
    ...
    <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-4">
        ...
        <form method="POST" class="grid grid-cols-1 sm:grid-cols-4 gap-4">
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">New Admin ID (Username) *</label>
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">Admin Email *</label>
...
<?php if ($isMainAdmin): ?>
<div id="editAdminModal" class="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md hidden items-center justify-center p-4">
    <div class="bg-white border border-slate-200 max-w-lg w-full rounded-3xl p-6 shadow-2xl space-y-6">
        <div class="flex items-center justify-between border-b border-slate-100 pb-4">
            ...
            <button type="button" onclick="closeEditAdminModal()" class="text-slate-400 hover:text-slate-600 font-bold text-xl">×</button>
        </div>
        ...
        <form method="POST" class="space-y-4">
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">Admin ID (Username)</label>
                <input type="text" id="modal_edit_username" disabled class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-100 text-slate-500 text-sm font-semibold cursor-not-allowed">
            </div>

            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">Display Name</label>
                <input type="text" name="edit_name" id="modal_edit_name" placeholder="Full Display Name" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-amber-500 outline-none">
            </div>

            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">Admin Email Address</label>
                <input type="email" name="edit_email" id="modal_edit_email" placeholder="admin@domain.com" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-amber-500 outline-none">
            </div>

            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">Reset Password (Optional)</label>
                <input type="password" name="edit_password" placeholder="Leave blank to keep existing password" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-amber-500 outline-none">
                <span class="text-[11px] text-slate-400 mt-1 block">Only enter a password if you want to reset this admin's credentials.</span>
            </div>

            <div class="flex justify-end gap-3 pt-2">
                <button type="button" onclick="closeEditAdminModal()" class="py-2.5 px-4 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl text-sm transition-all">Cancel</button>
                <button type="submit" class="py-2.5 px-6 gold-gradient text-slate-950 font-bold rounded-xl text-sm shadow-md hover:shadow-lg transition-all">Save Changes</button>
            </div>
        </form>
    </div>
</div>
