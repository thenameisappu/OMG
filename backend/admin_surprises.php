...
if (isset($_POST['action']) && $_POST['action'] === 'save_experience') {
    ...
    if ($title && $base_price > 0 && $image) {
        if ($id) {
            ...
            $stmt->execute([
                ':title' => $title, ':subtitle' => $subtitle, ':description' => $description, ':badge' => $badge,
                ':base_price' => $base_price, ':image' => $image, ':features' => $features_json,
                ':display_order' => $display_order, ':is_active' => $is_active, ':id' => $id
            ]);
    ...
    if ($name && $price >= 0) {
        if ($id) {
            ...
            $stmt->execute([
                ...
                ':price' => $price, ':display_order' => $display_order, ':is_active' => $is_active, ':id' => $id
            ]);
            ...
            $stmt->execute([
                ':name' => $name, ':description' => $description, ':icon' => $icon,
                ':price' => $price, ':display_order' => $display_order, ':is_active' => $is_active
            ]);
...
<div class="space-y-8">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        <div>
            ...
            <p class="text-slate-500 text-sm mt-1">Manage base surprise packages, upgrades, pricing, and Bengaluru delivery pincodes.</p>
        </div>
    ...
    <!-- Alert Messages -->
    <?php if ($message): ?>
        <div class="bg-emerald-50 border border-emerald-200 text-emerald-700 px-4 py-3 rounded-xl text-sm font-medium"><?php echo htmlspecialchars($message); ?></div>
    <?php endif; ?>
    <?php if ($error): ?>
        <div class="bg-rose-50 border border-rose-200 text-rose-700 px-4 py-3 rounded-xl text-sm font-medium"><?php echo htmlspecialchars($error); ?></div>
    <?php endif; ?>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 space-y-6">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-4">
            <div>
                ...
                <p class="text-xs text-slate-500">Add or edit base surprise packages, titles, pricing, badges and images.</p>
            </div>
            <button onclick="openExpModal()" class="py-2 px-4 gold-gradient text-slate-950 font-bold rounded-xl text-xs shadow-md hover:shadow-lg transition-all">
                + Add New Experience
            </button>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <?php foreach ($experiences as $exp): ?>
                ...
                ?>
                <div class="border border-slate-200 rounded-xl p-5 bg-slate-50/50 flex flex-col justify-between space-y-4 hover:border-amber-400 transition-colors">
                    <div class="space-y-3">
                        <div class="flex justify-between items-start gap-2">
                            <div>
                                <span class="px-2 py-0.5 text-[10px] uppercase font-bold tracking-wider rounded bg-slate-200 text-slate-700">Order #<?php echo $exp['display_order']; ?></span>
                                <?php if ($exp['badge']): ?>
                                    <span class="px-2 py-0.5 text-[10px] font-bold rounded bg-amber-100 text-amber-800 border border-amber-200 ml-1"><?php echo htmlspecialchars($exp['badge']); ?></span>
                                <?php endif; ?>
                                <h4 class="font-serif font-bold text-base text-slate-900 mt-1"><?php echo htmlspecialchars($exp['title']); ?></h4>
                            </div>
                            <span class="text-lg font-bold text-amber-600">₹<?php echo number_format($exp['base_price']); ?></span>
                        </div>
                        <p class="text-xs text-slate-500 italic"><?php echo htmlspecialchars($exp['subtitle']); ?></p>
                        <p class="text-xs text-slate-600 line-clamp-2"><?php echo htmlspecialchars($exp['description']); ?></p>
                        
                        <div class="flex items-center gap-3 pt-2">
                            <img src="<?php echo htmlspecialchars($exp['image']); ?>" class="w-14 h-14 object-cover rounded-lg border border-slate-200" alt="Exp Image">
                    ...
                    <div class="flex items-center justify-between pt-3 border-t border-slate-200/60 text-xs">
                        <span class="<?php echo $exp['is_active'] ? 'text-emerald-600 font-semibold' : 'text-slate-400'; ?>">
                            <?php echo $exp['is_active'] ? '● Active' : '○ Disabled'; ?>
                        </span>
                        <div class="flex items-center gap-2">
                            <button onclick='editExp(<?php echo json_encode($exp); ?>)' class="px-3 py-1 bg-slate-800 text-white rounded-lg font-medium hover:bg-slate-900">Edit</button>
                            <form method="POST" onsubmit="return confirm('Delete this experience?');" class="inline">
                                ...
                                <button type="submit" class="px-3 py-1 bg-rose-50 text-rose-600 rounded-lg font-medium hover:bg-rose-100">Delete</button>
                            </form>
                        </div>
                    </div>
                </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 space-y-6">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-4">
            <div>
                ...
                <p class="text-xs text-slate-500">Configure optional add-on experiences, icons, descriptions & prices.</p>
            </div>
            <button onclick="openUpgModal()" class="py-2 px-4 gold-gradient text-slate-950 font-bold rounded-xl text-xs shadow-md hover:shadow-lg transition-all">
                + Add New Upgrade
            </button>
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php foreach ($upgrades as $upg): ?>
                        <tr class="hover:bg-slate-50">
                            <td class="py-3.5 px-4 text-xs font-mono font-bold text-slate-500">#<?php echo $upg['display_order']; ?></td>
                            <td class="py-3.5 px-4 font-semibold text-amber-600 text-xs"><?php echo htmlspecialchars($upg['icon']); ?></td>
                            <td class="py-3.5 px-4 font-bold text-slate-900"><?php echo htmlspecialchars($upg['name']); ?></td>
                            <td class="py-3.5 px-4 text-xs text-slate-600 max-w-xs"><?php echo htmlspecialchars($upg['description']); ?></td>
                            <td class="py-3.5 px-4 font-bold text-amber-600">₹<?php echo number_format($upg['price']); ?></td>
                            <td class="py-3.5 px-4 text-xs">
                                <span class="<?php echo $upg['is_active'] ? 'text-emerald-600 font-semibold' : 'text-slate-400'; ?>">
                                    <?php echo $upg['is_active'] ? 'Active' : 'Disabled'; ?>
                                </span>
                            </td>
                            <td class="py-3.5 px-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button onclick='editUpg(<?php echo json_encode($upg); ?>)' class="px-2.5 py-1 bg-slate-800 text-white rounded text-xs font-medium">Edit</button>
                                    <form method="POST" onsubmit="return confirm('Delete this upgrade?');" class="inline">
                                        ...
                                        <button type="submit" class="px-2.5 py-1 bg-rose-50 text-rose-600 rounded text-xs font-medium">Delete</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 space-y-4">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-4">
            <div>
                ...
                <p class="text-xs text-slate-500">Live delivery area validation is powered by Google Maps Geocoding API.</p>
            </div>
        ...
        <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-2 text-xs text-slate-600">
            ...
            <ul class="list-disc list-inside space-y-1 text-slate-500">
                ...
                <li>The backend validates pincodes via the Google Maps Geocoding API using <code class="font-mono text-amber-700 bg-amber-50 px-1 py-0.5 rounded">GOOGLE_MAPS_API_KEY</code> configured in <code class="font-mono text-slate-800 font-bold">.env</code>.</li>
                <li>If the geocoded location resides within <strong>Bengaluru (Bangalore)</strong>, delivery is accepted as <em>"Delivery Available"</em>.</li>
                <li>If outside Bengaluru or invalid, the user is informed: <em>"Sorry, we currently deliver only within Bengaluru."</em></li>
            </ul>
        </div>
    </div>
</div>
...
<div id="expModal" class="hidden fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4">
    <div class="bg-white max-w-lg w-full rounded-2xl p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
        <h3 id="expModalTitle" class="text-lg font-serif font-bold text-slate-900 border-b border-slate-100 pb-3">Add Experience</h3>
        <form method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" id="exp_title" name="title" required class="w-full px-4 py-2 rounded-xl border border-slate-300 text-sm outline-none focus:ring-2 focus:ring-amber-500">
            </div>
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Subtitle</label>
                <input type="text" id="exp_subtitle" name="subtitle" class="w-full px-4 py-2 rounded-xl border border-slate-300 text-sm outline-none focus:ring-2 focus:ring-amber-500">
            </div>
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Description</label>
                <textarea id="exp_description" name="description" rows="3" class="w-full px-4 py-2 rounded-xl border border-slate-300 text-sm outline-none focus:ring-2 focus:ring-amber-500"></textarea>
            </div>
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Base Price (₹)</label>
                    <input type="number" step="0.01" id="exp_price" name="base_price" required class="w-full px-4 py-2 rounded-xl border border-slate-300 text-sm outline-none focus:ring-2 focus:ring-amber-500">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Badge (e.g. Top Rated)</label>
                    <input type="text" id="exp_badge" name="badge" class="w-full px-4 py-2 rounded-xl border border-slate-300 text-sm outline-none focus:ring-2 focus:ring-amber-500">
                </div>
            </div>
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Image URL</label>
                <input type="url" id="exp_image" name="image" required class="w-full px-4 py-2 rounded-xl border border-slate-300 text-sm outline-none focus:ring-2 focus:ring-amber-500">
            </div>
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Features List (1 per line)</label>
                <textarea id="exp_features" name="features" rows="4" placeholder="2-Hour Setup&#10;Rose Petal Carpet" class="w-full px-4 py-2 rounded-xl border border-slate-300 text-sm outline-none focus:ring-2 focus:ring-amber-500"></textarea>
            </div>
            <div class="grid grid-cols-2 gap-3 items-center">
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Display Order</label>
                    <input type="number" id="exp_order" name="display_order" value="1" class="w-full px-4 py-2 rounded-xl border border-slate-300 text-sm outline-none focus:ring-2 focus:ring-amber-500">
                </div>
                <div class="pt-4">
                    <label class="flex items-center gap-2 cursor-pointer font-semibold text-sm text-slate-700">
                        <input type="checkbox" id="exp_active" name="is_active" value="1" checked class="w-4 h-4 text-amber-600 rounded"> Active on Site
                    </label>
                </div>
            </div>

            <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
                <button type="button" onclick="closeExpModal()" class="px-4 py-2 text-slate-600 text-sm font-semibold">Cancel</button>
                <button type="submit" class="px-5 py-2 gold-gradient text-slate-950 font-bold rounded-xl text-sm shadow-md">Save Experience</button>
            </div>
        </form>
    </div>
</div>
...
<div id="upgModal" class="hidden fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4">
    <div class="bg-white max-w-md w-full rounded-2xl p-6 shadow-2xl space-y-4">
        <h3 id="upgModalTitle" class="text-lg font-serif font-bold text-slate-900 border-b border-slate-100 pb-3">Add Upgrade</h3>
        <form method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" id="upg_name" name="name" required class="w-full px-4 py-2 rounded-xl border border-slate-300 text-sm outline-none focus:ring-2 focus:ring-amber-500">
            </div>
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Description</label>
                <textarea id="upg_description" name="description" rows="2" class="w-full px-4 py-2 rounded-xl border border-slate-300 text-sm outline-none focus:ring-2 focus:ring-amber-500"></textarea>
            </div>
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Price (₹)</label>
                    <input type="number" step="0.01" id="upg_price" name="price" required class="w-full px-4 py-2 rounded-xl border border-slate-300 text-sm outline-none focus:ring-2 focus:ring-amber-500">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Icon Name</label>
                    <select id="upg_icon" name="icon" class="w-full px-4 py-2 rounded-xl border border-slate-300 text-sm outline-none focus:ring-2 focus:ring-amber-500 bg-white">
            ...
            <div class="grid grid-cols-2 gap-3 items-center">
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Display Order</label>
                    <input type="number" id="upg_order" name="display_order" value="1" class="w-full px-4 py-2 rounded-xl border border-slate-300 text-sm outline-none focus:ring-2 focus:ring-amber-500">
                </div>
                <div class="pt-4">
                    <label class="flex items-center gap-2 cursor-pointer font-semibold text-sm text-slate-700">
                        <input type="checkbox" id="upg_active" name="is_active" value="1" checked class="w-4 h-4 text-amber-600 rounded"> Active on Site
                    </label>
                </div>
            </div>

            <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
                <button type="button" onclick="closeUpgModal()" class="px-4 py-2 text-slate-600 text-sm font-semibold">Cancel</button>
                <button type="submit" class="px-5 py-2 gold-gradient text-slate-950 font-bold rounded-xl text-sm shadow-md">Save Upgrade</button>
            </div>
        </form>
    </div>
</div>
